...
# This function will output multiple files including CellType_CombinedSignedNegLogPValranksNormalized_DirwinHallPValues_Top_ProportionofTopDatasets_GSEAPathwayRanks.txt, which includes p-values for all pathways based on their reproducibility.
...
SumRank_GSEAPathways <- function(DatasetNames,BroadClusterTypes,SuffixofDifferentialExpressionOutput="",CommonGenes,ProportionofTopDatasets,PresenceofDataTable,directory,m_df_gene2term){
...
### Get final pathway list (all pathways present in all datasets)
i=1
j=1
currentTest <- get(paste0(DatasetNames[i],"_",BroadClusterTypes[j],"_SignedNegLogPVal_ranked_GSEAPathwayRanks"))
finalpathwaylist = currentTest$ID

for(i in 1:length(DatasetNames)){
  for(j in 1:length(BroadClusterTypes)){
    if(PresenceofDataTable[PresenceofDataTable$Dataset==DatasetNames[i],][BroadClusterTypes[j]][[1]]==1){
      currentTest <- get(paste0(DatasetNames[i],"_",BroadClusterTypes[j],"_SignedNegLogPVal_ranked_GSEAPathwayRanks"))
      finalpathwaylist = finalpathwaylist[finalpathwaylist %in% currentTest$ID]
}}}
FinalPathwayList_Table = as.data.frame(finalpathwaylist)
write.table(FinalPathwayList_Table, file=paste("GSEAPathways_FinalPathwayListTable.txt",sep=""),sep="\t",row.names=FALSE,col.names=TRUE,quote=FALSE)
