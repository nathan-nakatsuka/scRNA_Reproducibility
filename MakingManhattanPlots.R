...
# Desiredggtitle is the title of the ggplot.
MakeManhattanPlot <- function(CalibratedPValuesTable, OtherNegLogPValueCutoff,TopValueCutoff, Desiredggtitle){
  ...
  CalibratedPValuesTable[is.infinite(CalibratedPValuesTable$New_NegLogPValue),5] <- as.numeric(TopValueCutoff)
  ...
  plot(CalibratedPValuesTable$PlotPoint,CalibratedPValuesTable$New_NegLogPValue,ylim=c(0,(as.numeric(TopValueCutoff)+0.2),pch=20,xaxt='n',xlab="Genes",ylab=expression("-log"[10]*"(p-value)"),main=Desiredggtitle,cex.main=0.6)
